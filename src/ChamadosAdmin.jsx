...
export default function ChamadosAdmin({ onOpenDetails }) {
  ...
  const getDate = (item) => item.data || item.created_at || item.data_abertura || item.date || item.criado_em || null;
  ...
  const getStatusStyle = (status) => {
    switch (status) {
      ...
      case 'Pendência Concluída': return 'bg-purple-500/10 text-purple-400';
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-200 dark:border-white/5 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col min-w-0">
                        ...
                        <span className="text-[9px] text-slate-400 dark:text-[#606060] font-extrabold uppercase mt-0.5">{new Date(getDate(item)).toLocaleDateString('pt-BR')}</span>
                      </div>
                    </div>
                  ...
                  <tr key={item.id} onClick={() => onOpenDetails(item)} className="hover:bg-[var(--bg-page)] dark:hover:bg-[var(--bg-card)]/[0.02] transition-colors group cursor-pointer">
                    ...
                    <td className="px-8 py-4 text-right">
                      <div className="flex flex-col items-end">
                        <span className="text-xs font-bold text-slate-900 dark:text-white">{new Date(getDate(item)).toLocaleDateString('pt-BR')}</span>
                        <span className="text-[10px] font-bold text-slate-500 dark:text-[#606060] mt-0.5">{new Date(getDate(item)).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</span>
                      </div>
                    </td>
                  </tr>
                ))}
